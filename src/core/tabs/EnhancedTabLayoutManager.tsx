...
export function EnhancedTabLayoutProvider({
    children,
}: {
    children: ReactNode;
}) {
    ...
    useEffect(() => {
        if (userSettings.layouts.length > 0 && !userSettings.isLoading) {
            ...
            if (userSettings.settings?.defaultLayout) {
                const defaultLayout = redisLayouts.find(
                    (l) => l.id === userSettings.settings?.defaultLayout
                );
                if (defaultLayout) {
    ...
    useEffect(() => {
        if (activeTabId && currentLayout && user?.id) {
            const activeTab = currentLayout.tabs.find(
                (t) => t.id === activeTabId
            );
            if (activeTab) {
