...
export class TabManager {
    ...
    static addComponent(
        ...
        if (!this.addTabFunction) {
            return {
                success: false,
                message:
                    "Tab system not initialized. Please wait for the application to load.",
            };
        }

        // Enable Portfolio and other enhanced registry components
        ...
        if (!availableComponents.includes(componentName)) {
            return {
                success: false,
                message: `Component '${componentName}' not available. Available: ${availableComponents.join(
                    ", "
                )}`,
            };
        }

        // Create new tab with the component
        const newTab = {
            name: customOptions?.tabName || componentName,
            component: componentName,
            type: "static" as const,
            icon: customOptions?.icon || this.getDefaultIcon(componentName),
            closable: true,
        ...
        try {
            this.addTabFunction(newTab);
            return {
                success: true,
                message: `✅ ${componentName} tab added successfully!`,
            };
        } catch (error) {
            return {
                success: false,
                message: `Failed to add ${componentName}: ${
    ...
    // Get default icon for component
    static getDefaultIcon(componentName: string): string {
        const iconMap: Record<string, string> = {
            Portfolio: "briefcase",
            TradingApp: "trending-up",
            Analytics: "bar-chart-2",
            Documentation: "book-open",
        };
        return iconMap[componentName] || "file";
    }

    // List all available components
    static listAvailableComponents(): string[] {
        return ["Portfolio", "TradingApp", "Analytics", "Documentation"];
    }
...
export function setupConsoleHelpers() {
    if (typeof window !== "undefined") {
        ...
        window.tabHelp = () => {
            ...
            console.log(
                '  addTab("name", {tabName: "..."})   - Add with custom name'
            );
            console.log(
                '  addTab("name", {icon: "🚀"})       - Add with custom icon'
            );
            console.log(
                "  listComponents()                   - Show all available components"
            );
            console.log(
                '  componentInfo("name")              - Get component details'
            );
            console.log(
                "  tabHelp()                          - Show this help"
            );
            console.log("");
